...
# Why?  Todo
  def test_parentheses_also_capture_matched_content_by_number
    assert_equal "Gray", "Gray, James"[/(\w+), (\w+)/, 1]  #shouldn't this match "Gray, James"
...
# Why? Todo
  def test_variables_can_also_be_used_to_access_captures
    ...
    assert_equal "Gray", $1       #I have no idea what's happening here; where was $1 set?
...
#Why?  todo #assume t\w* finds all occurrences of words that start with t (ignores space and -).  Sub that word with "t".  { $1[0,1] } does what?
  def test_sub_is_like_find_and_replace
    assert_equal "one t-three", "one two-three".sub(/(t\w*)/) { $1[0, 1] }
  end

#Why?  todo # see def test_sub_is_like_find_and_replace
